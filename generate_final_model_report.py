...
METRICS_PATH = Path("results/metrics/2step_psychological_correction_summary.csv")
FIGURES_DIR = Path("results/figures/final_model")
...
def build_company_section(df: pd.DataFrame, company: str) -> str:
    ...
    lines = []
    lines.append(f"<section class=\"card\"><div class=\"section-title\"><span></span><h3>{company.replace('_', ' ').title()}</h3></div>")
    lines.append("<p>Baseline / Corrected 모델 결과를 비교한 지표와 최종 시각화를 확인하세요.</p>")

    lines.append("<div class=\"figure-card\">")
    img_path = f"{company}_2step_correction.png"
    if (FIGURES_DIR / img_path).exists():
        lines.append(f"<img src=\"{img_path}\" alt=\"{company} 2step correction chart\">")
        lines.append(f"<div class=\"image-caption\">{company.replace('_', ' ').title()} 최종 모델 비교</div>")
    lines.append("</div>")

    lines.append("<table class=\"metric-table\"><thead><tr><th>Model</th><th>RMSE</th><th>MAE</th><th>MAPE</th><th>MBE</th><th>R²</th><th>Directional Accuracy</th></tr></thead><tbody>")
...
def main():
    ...
    for company in sorted(df['Company'].unique()):
        company_sections.append(build_company_section(df, company))
